    ...
    return(
        <>
            <div className="school-private">
                <button className={`swap-btn school-btn ${activeButton === "School" ? "active" : ""}`} onClick={schoolClick}>{button1Text}</button>
                <button className={`swap-btn private-btn ${activeButton === "Private" ? "active" : ""}`} onClick={privateClick}>{button2Text}</button>
            </div>
        </>
    )
}
